...
const createDbSchema = (obj) => {
  ...
  obj && obj.forEach(item => {
    ...
    if (typeof rules.validator === 'function') {
      mgschema[key].validate = function (val) {
        debug('execute validator...', rules.validator);
        return rules.validator.call(this, rules, val, this, {}, {});
      }
    } else if (typeof rules.validator === 'string') {
      let fn;
      if (_.isArray(rules.validator)) {
        if (rules.validator.every(it => it.indexOf('return ') === -1)) {
          fn = rules.validator.map((it, i) => i === rules.validator.length - 1 ? 'return ' + it : it).join('\n');
        } else {
          fn = rules.validator.join('\n');
        }
      } else {
        if (rules.validator.indexOf('return ') === -1) {
          fn = 'return ' + rules.validator;
        } else {
          fn = rules.validator;
        }
      }
      mgschema[key].validate = function (val) {
        ...
        return new Function('rule, value, source, options, scope', fn).call(this, rules, val, this, {}, {});
      }
    }
