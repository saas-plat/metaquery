...
const getType = exports.getType = (type) => {
  const Model = tables.get(type) || MetaTable;
  return Model;
}
...
const MetaTable = exports.MetaTable = class MetaTable extends mongoose.Model {
  ...
  static createModel(name, meta, opts = {}) {
    if (!name) {
    ...
    const metaschema = MetaTable.createSchema(BaseTable, meta);
    const sche = createDbSchema(metaschema.fields);
    sche._ns = {
      type: String,
      index: true,
    }
    ...
    schema.set('schema', metaschema);
    schema.set('version', version);
    schema.set('description', description);

    // 查询模式是给gql提供生成schema用
    if (!query) {
      if (metaschema.conflicts.length > 0) {
        throw new Error(i18n.t('系统字段{{keys}}冲突，请修改成其他名称!', {
          keys: metaschema.conflicts.map(it => it.key).join(',')
        }))
      }

      // 需要mapto，doc.set即可
      //createMapping(fields, name);
      createVirtualProps(schema, metaschema.mappings);
    ...
    if (populateReferences) {
      ...
      post('find', async function (docs) {
        await populateReferencesHandler(metaschema.references, populateServiceFn.bind(this), ...docs);
        debug(docs)
      }).
      post('findOne', async function (doc) {
        await populateReferencesHandler(metaschema.references, populateServiceFn.bind(this), doc);
      }).
