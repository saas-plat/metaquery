...
exports.DataMigration = class DataMigration {
  ...
  async up(rules, opts = {
    ...
    for (const tableType of this.migrationTypes) {
      ...
      await this._migrationTables(tableType);
    }
  }

  async _migrationTables(model) {
    ...
    while (doc) {
      ...
      if (!NewModel) {
        // 执行自定义规则
        await this._migrateAction(actions, oldData);
        ...
        // 执行自定义规则
        await this._migrateAction(actions, oldData, upDoc);
        // 需要更新
        if (upDoc.isModified()) {
          isUpdated = true;
        }
  ...
  async _migrateAction(actions, data, doc) {
    if (!this._onAction) {
      return eit.data;
    }
