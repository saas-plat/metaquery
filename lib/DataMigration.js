...
exports.DataMigration = class DataMigration {
  ...
  async _migrationTables(model, ruleset) {
    const scopes = [model.schema.get('name'), model.schema.get('schema').type, ''];
    if (model.schema.get('ns')) {
      scopes.push(
        model.schema.get('ns') + '.' + model.schema.get('name'),
        model.schema.get('ns') + '.' + model.schema.get('schema').type);
    }
    const batchSize = 10;
